...
internal static class CriHcaTables
{
    ...
    static CriHcaTables()
    {
        var arrays = ArrayUnpacker.UnpackArrays(PackedTables);
    ...
    private static double DequantizerScalingFunction(int x)
    {
        return Math.Sqrt(128) * Math.Pow(Math.Pow(2, 53.0 / 128), x - 63);
    }
    ...
    private static double QuantizerInverseStepSizeFunction(int x)
    {
        return ResolutionMaxValueFunction(x) + 0.5;
    }

    private static double ScaleConversionTableFunction(int x)
    {
        return x > 1 && x < 127 ? Math.Pow(Math.Pow(2, 53.0 / 128), x - 64) : 0;
    }

    private static double IntensityRatioFunction(int x)
    {
        return (28 - x * 2) / 14.0;
    }

    private static double IntensityRatioBoundsFunction(int x)
    {
        return (27 - x * 2) / 14.0;
    }
    ...
    private static double QuantizerDeadZoneFunction(int i)
    {
        ...
        var steps = ResolutionMaxValueFunction(i) + 1;
        var boundary = QuantizerStepSizeFunction(i) / 2;
